    ...
    if 2 < len(message.text) < 100:    
        ...
        if files:
            for file in files:
                ...
                btn.append(
                    [InlineKeyboardButton(text=f"{filename}",{file_size}",callback_data=f"pr0fess0r_99#{file_id}")]
                    )
        else:
    ...
    if 2 < len(message.text) < 50:    
        ...
        if files:
            for file in files:
                ...
                filesize = f" [{get_size(file.file_size)}]"
                btn.append(
                    [InlineKeyboardButton(text=f"{filename}",{file_size}", url=f"https://telegram.dog/{nyva}?start=pr0fess0r_99_-_-_-_{file_id}")]
                )
        else:
            return
        if not btn:
